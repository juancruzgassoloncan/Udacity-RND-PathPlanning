...
//Solution using navigation stack http://wiki.ros.org/navigation/Tutorials/SendingSimpleGoals
...
bool handlePickObj(float p_x, float p_y, float p_orientation) {
  bool result = false;
...
while(!ac.waitForServer(ros::Duration(5.0))){
  ...
  move_base_msgs::MoveBaseGoal robot;

  robot.target_pose.header.frame_id = "map";
  robot.target_pose.header.stamp = ros::Time::now();

  // Define a goal position
  robot.target_pose.pose.position.x = p_x;
  robot.target_pose.pose.position.y = p_y;
  robot.target_pose.pose.orientation.w = p_orientation;
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {
    ...
    ROS_INFO("ERROR. The robotdoesnt reach the goal!");
    result = false;
  }
...
int main( int argc, char** argv )
{
  float pick_x = 4.5;
  float pick_y = 3;
  float pick_orientation = 1;

  float drop_x = -2.0;
  float drop_y= -0.1;
  float drop_orientation = 1;
  ...
  move_base_result = handlePickObj(pick_x, pick_y,pick_orientation);
 ...
 // Deletion of first marker after reach the coord.
  if(move_base_result) {
  ...
  move_base_result = handlePickObj(drop_x, drop_y, drop_orientation);
